...
function updatePacketPaths() {
    ...
    if (!packet.lost && !packet.stopped) {
      for (let i = 0; i < points.length - 1; i++) {
        ...
        let latency = edge ? edge.weight : 100;
          let lossProb = map(latency, 100, 600, 0.0005, 0.005, true); // much lower loss rate
  ...
  packets.forEach(packet => {
    ...
    let speed = map(totalLatency, 400, 2200, 0.18, 0.07, true);
...
function drawSSID() {
  ...
  fill(0, 180);
  rectMode(CENTER);
  rect(width / 2, height - 38, 320, 38, 10);
  fill(255, 220);
  textSize(16);
  text('SSID: ' + ssid, width / 2, height - 44);
  fill(255, 120);
  textSize(12);
  text('-67dBm  |  Virtual APs: ' + VAP_COUNT + '  |  Routers: ' + ROUTER_COUNT + '  |  Traceroutes: ' + routes.length, width / 2, height - 24);
  pop();
}
